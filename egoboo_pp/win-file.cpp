...
void fs_init()
{
  ...
  strncat(win32_importPath, "import\\", MAX_PATH);
  ...
  if (hFile == NULL)
  {
    ...
    if (hFile == NULL)
    {
      ...
      log_error("fs_init: Could not find basicdat directory!\n");
    }
  }
  CloseHandle(hFile);
...
const char *fs_findFirstFile(const char *searchDir, const char *searchExtension)
{
  ...
  {
    snprintf(searchSpec, MAX_PATH, "%s\\", searchDir);
  }
  else
  {
  ...
  {
    snprintf(searchSpec, MAX_PATH, "%s*.%s", searchSpec, searchExtension);
  }
  else
  {
...
void empty_import_directory(void)
{
  ...
  snprintf(searchName, MAX_PATH, "import\\*.obj");
  ...
  while (hFind != NULL && hFind != INVALID_HANDLE_VALUE)
  {
    ...
    if (fileName[0] != '.')
    {
      snprintf(filePath, MAX_PATH, "import\\%s", fileName);
