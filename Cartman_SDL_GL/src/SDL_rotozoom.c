...
int zoomSurfaceRGBA(SDL_Surface * src, SDL_Surface * dst, int smooth)
{
    ...
    */
    if ((sax = calloc( sizeof(Uint32), (dst->w + 1) )) == NULL)
    {
        return (-1);
    }
    if ((say = calloc( sizeof(Uint32), (dst->h + 1) )) == NULL)
    {
        free(sax);
        return (-1);
    }
...
int zoomSurfaceY(SDL_Surface * src, SDL_Surface * dst)
{
    ...
    * Allocate memory for row increments
    */
    if ((sax = calloc( sizeof(Uint32), dst->w )) == NULL)
    {
        return (-1);
    }
    if ((say = calloc( sizeof(Uint32), dst->h )) == NULL)
    {
        if (sax != NULL)
        {
            free(sax);
