...
void render_enviromad( Uint16 character, Uint8 trans )
{
    ...
    Uint8 lightrotation = FP8_TO_INT( chrturnleftright[character] + chrlightturnleftright[character] );
    Uint32  alpha = trans;
    Uint8 lightlevel = chrlightlevel[character] >> 4;
    float uoffset = textureoffset[ FP8_TO_INT( chruoffset[character] )] - camturnleftrightone;
    float voffset = textureoffset[ FP8_TO_INT( chrvoffset[character] )];
    ...
    */
    ...
    glPushMatrix();
    glMultMatrixf( chrmatrix[character] );
...
//  DWORD fogspec;
    ...
    float uoffset = textureoffset[ FP8_TO_INT( chruoffset[character] )];
    float voffset = textureoffset[ FP8_TO_INT( chrvoffset[character] )];
    Uint8 rs = chrredshift[character];
    Uint8 gs = chrgrnshift[character];
    Uint8 bs = chrblushift[character];
