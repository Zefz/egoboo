...
void render_antialias_prt()
{
    ...
    vector_right.x = cam.mView.v[0];
    vector_right.y = cam.mView.v[4];
    vector_right.z = cam.mView.v[8];
    vector_up.x = cam.mView.v[1];
    vector_up.y = cam.mView.v[5];
    vector_up.z = cam.mView.v[9];
...
void render_refprt()
{
    ...
    while ( cnt < maxparticles )
    {
        ...
        {
            if ( meshfx[prtonwhichfan[cnt]] & MPDFX_DRAWREF )
            {
                level = prtlevel[cnt];
                v[numparticle].x = ( float ) prtxpos[cnt];
                v[numparticle].y = ( float ) prtypos[cnt];
