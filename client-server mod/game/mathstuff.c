...
Uint16 randie_table[RANDIE_MAX];
Uint16 randie_index  = 0;
Uint32 ego_rand_seed = (Uint32)(-1);
...
{
  float cx = turntosin[(turnx+4096) & TRIGTABLE_MASK];
  float sx = turntosin[turnx & TRIGTABLE_MASK];
  float cy = turntosin[(turny+4096) & TRIGTABLE_MASK];
  float sy = turntosin[turny & TRIGTABLE_MASK];
  float cz = turntosin[(turnz+4096) & TRIGTABLE_MASK];
...
void make_randie()
{
  ...
  cnt = 0;
  while (cnt < RANDIE_MAX)
  {
    randie_table[cnt] = ego_rand(&ego_rand_seed) << 1;
    cnt++;
  }
  ...
  while (tnc < 20)
  {
    ...
    while (cnt < RANDIE_MAX)
    {
      randie_table[cnt] += (Uint16)ego_rand(&ego_rand_seed);
      cnt++;
    }
    tnc++;
  }
...
Uint32 ego_rand(Uint32 * seed)
{
  ...
  (*seed) = ((*seed) * 0x0019660DL) + 0x3C6EF35FL;
