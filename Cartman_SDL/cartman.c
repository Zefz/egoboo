...
SDLX_video_parameters_t vparam;
...
void load_all_windows(void)
{
    ...
    load_window(0, "data" SLASH_STR "window.pcx", 180, 16,  7, 9, 200, 200, WINMODE_VERTEX);
    load_window(1, "data" SLASH_STR "window.pcx", 410, 16,  7, 9, 200, 200, WINMODE_TILE);
    load_window(2, "data" SLASH_STR "window.pcx", 180, 248, 7, 9, 200, 200, WINMODE_SIDE);
    load_window(3, "data" SLASH_STR "window.pcx", 410, 248, 7, 9, 200, 200, WINMODE_FX);
}
...
void load_img(void)
{
    ...
    bmptemp = cartman_LoadIMG("data" SLASH_STR "point.pcx");
    ...
    bmptemp = cartman_LoadIMG("data" SLASH_STR "pointon.pcx");
    for (cnt = 0; cnt < MAXPOINTSIZE; cnt++)
    {
    ...
    imgref     = cartman_LoadIMG("data" SLASH_STR "ref.pcx");
    imgdrawref = cartman_LoadIMG("data" SLASH_STR "drawref.pcx");
    imganim    = cartman_LoadIMG("data" SLASH_STR "anim.pcx");
    imgwater   = cartman_LoadIMG("data" SLASH_STR "water.pcx");
    imgwall    = cartman_LoadIMG("data" SLASH_STR "slit.pcx");
    imgimpass  = cartman_LoadIMG("data" SLASH_STR "impass.pcx");
    imgdamage  = cartman_LoadIMG("data" SLASH_STR "damage.pcx");
    imgslippy  = cartman_LoadIMG("data" SLASH_STR "slippy.pcx");
}
...
int main(int argcnt, char* argtext[])
{
    ...
    gFont = TTF_OpenFont( "data" SLASH_STR "pc8x8.fon", 12 );
...
void sdlinit( int argc, char **argv )
{
    ...
    vparam.flags          = 0;
    vparam.opengl         = SDL_FALSE;
    vparam.doublebuffer   = SDL_TRUE;
    vparam.glacceleration = GL_FALSE;
    vparam.width          = MIN( 640, cfg.scr.x );
    vparam.height         = MIN( 480, cfg.scr.y );
    vparam.depth          = cfg.scr.d;

    // Get us a video mode
    if ( NULL == SDLX_set_mode(NULL, &vparam) )
    {
