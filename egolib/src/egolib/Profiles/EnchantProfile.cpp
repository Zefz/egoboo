...
std::shared_ptr<EnchantProfile> EnchantProfile::readFromFile(const std::string& pathname)
{
    ...
    if ('R' == Ego::toupper(cTmp)) profile->_set[EnchantProfile::SETMISSILETREATMENT].value = MISSILE_REFLECT;
    else if ('D' == Ego::toupper(cTmp)) profile->_set[EnchantProfile::SETMISSILETREATMENT].value = MISSILE_DEFLECT;
    else profile->_set[EnchantProfile::SETMISSILETREATMENT].value = MISSILE_NORMAL;
